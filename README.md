...
ChinaTravel 是一个面向海外游客的中国旅行 OTA Web 示例项目。项目已经完成 `PRODUCT_ROADMAP.md` 中的一期 OTA 商品化 MVP、二期搜索转化/信任体系，以及三期行程规划与购物车闭环：从目的地灵感展示升级为可搜索、可筛选、可选择套餐/日期/人数/优惠券、可创建商品订单、可生成行程、可加入购物车并打包下单、可管理订单状态并写 verified review 的 OTA 交易雏形。

## 三期状态

- **阶段**：三期 行程规划与购物车闭环 已完成
- **定位**：China Travel Super App Web 版雏形
- **核心闭环**：首页商品频道 -> 高级商品搜索 -> 商品详情信任/评价 -> 加入行程/加入购物车/立即预订 -> AI 生成 day-by-day 行程 -> Trips 时间线排序 -> 购物车多商品打包下单 -> 商品订单售后与评价
- **兼容策略**：保留旧 `/bookings` 简易预订接口，新 OTA 交易走 `/orders`、`/coupons`、`/reviews`、`/itineraries`、`/cart`

## 技术栈

- **前端**：Vue 3、Vite、Vue Router、Vue I18n、Vitest
- **后端**：Go 1.21、标准库 `net/http`、SQLite 驱动 `github.com/mattn/go-sqlite3`
- **数据存储**：SQLite 为商品、套餐、库存、订单、优惠券、评价、行程、购物车等 OTA 状态的主存储；JSON/缓存继续服务部分演示数据
